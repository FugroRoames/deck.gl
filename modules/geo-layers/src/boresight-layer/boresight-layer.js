...
export default class BoresightLayer extends CompositeLayer {
  initializeState() {
    ...
    if (groundPointUrl) {
      ...
      groundPointPromise.then(geojson => {
        this.setState({gcpData: geojson.features});
      });
    }
  }
  ...
  /* eslint-disable complexity, max-statements */
  renderLayers() {
    const {bounds, gcpData, colorTexture, colorDomainTexture} = this.state;
    ...
    for (const dataUrl in data) {
      ...
      const pointColor = parameters.pointColor;

      const layer = new Roames3DLayer({
    ...
    const textures = [];
    const currentSubLayers = this.getSubLayers();

    for (const i in currentSubLayers) {
      if (currentSubLayers[i] instanceof Roames3DLayer) {
        const texture = currentSubLayers[i].getTexture();
    ...
    if (textures.length >= 2 && heightDiffTexture) {
      subLayers.push(
        new TriangleLayer(
          {
